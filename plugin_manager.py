...
class PluginManager:
    ...
    def handle_message(self, message, **kwargs):
        ...
        def _filter_kwargs_for_func(func, extra_kwargs):
            """仅保留函数签名里存在的关键字参数，避免向旧插件传入未定义参数（如 send）"""
            try:
                sig = inspect.signature(func)
                return {k: v for k, v in extra_kwargs.items() if k in sig.parameters and v is not None}
            except Exception:
                return {}
                    ...
                    if parameter or parameter_quantity > 0:
                        if parameter == "message_id":
                            ...
                            if target_message_id:
                                extra = {"group_id": kwargs.get("group_id"), "send": kwargs.get("send"), "revoke": kwargs.get("revoke")}
                                func(target_message_id, **_filter_kwargs_for_func(func, extra))
                            else:
                            ...
                            if parameter_quantity > 0:
                                ...
                                if parameter_quantity == 1:
                                    extra = {"group_id": kwargs.get("group_id"), "send": kwargs.get("send"), "revoke": kwargs.get("revoke")}
                                    func(param_text, **_filter_kwargs_for_func(func, extra))
                                    ...
                                    # 将group_id插入到适当位置（通常是第二个位置）
                                    group_id = kwargs.get('group_id')
                                    if group_id is not None:
                                        params.insert(1, group_id)  # 在第二个位置插入group_id
                                    extra = {"group_id": kwargs.get("group_id"), "send": kwargs.get("send"), "revoke": kwargs.get("revoke")}
                                    func(*params, **_filter_kwargs_for_func(func, extra))
                            else:
                                extra = {"group_id": kwargs.get("group_id"), "send": kwargs.get("send"), "revoke": kwargs.get("revoke")}
                                func(**_filter_kwargs_for_func(func, extra))
                    else:
                        # 对于没有参数的函数，只传递兼容的关键字
                        extra = {"group_id": kwargs.get("group_id"), "send": kwargs.get("send"), "revoke": kwargs.get("revoke")}
                        func(**_filter_kwargs_for_func(func, extra))
