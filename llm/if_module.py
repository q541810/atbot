...
async def llm_if(message: str, bot_name: str, bot_qq: int, url: str, key: str, model: str, message_content: str, bot_Prompt: str, 消息记录: str = ""):
    ...
    if "@" in message and str(bot_name) in message:
        ...
        completion = await client.chat.completions.create(
            ...
            max_tokens=2,
            messages=[
                {"role": "system", "content": f"你叫{bot_name}，消息记录:{消息记录}请输出你对{message}的感兴趣的程度且只输出这段数字(0~10),不能带有其他的任何的东西!不能换行！只能输出一个数字！。你的人设：{bot_Prompt}，认为和你没太大关系就可以填0，为6就代表你认为大概可以回复，为8就代表你觉得没问题，为10就代表你认为必须。回复判断标准：如果消息直接提到机器人或询问问题，应该回复。 如果消息是日常闲聊且适合参与，并且你认为他会感兴趣，可以回复。 如果消息是纯表情、无意义内容或私人对话，不应回复（你可以按你自己的判断输出0~10的任意一个数字）"}
            ]
        )
        # 获取返回内容并处理
