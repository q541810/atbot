...
def 查询用户名(qq号, napcat_host, napcat_port):
    ...
    try:
        ...
        if nick:
            info(f"记录了用户qq号为{qq号}的用户名{nick}")
            return nick
    except Exception as e:
        info(f"查询用户名失败: {e}")
    return None
...
async def 查询用户名_ws(qq号: int, napcat_host: str, napcat_port: int, group_id: Optional[int] = None) -> Optional[str]:
    ...
    from log import info, warning
    if group_id:
        resp = await _onebot_ws_action("get_group_member_info", {"group_id": group_id, "user_id": qq号}, napcat_host, napcat_port, f"get_group_member_info_{group_id}_{qq号}")
        if resp and resp.get("status") == "ok":
            ...
            if nick:
                ...
                return nick
    resp = await _onebot_ws_action("get_stranger_info", {"user_id": qq号}, napcat_host, napcat_port, f"get_stranger_info_{qq号}")
    if resp and resp.get("status") == "ok":
        data = resp["data"]
